...
def parse_argument_list(tokens):
    ...
    while tokens[0]["tag"] != ")":
        ...
        assert tokens[0]["tag"] in [",", ")"], "Error: expected ',' or ')'"
        tokens = tokens[1:]
...
def test_parse_simple_expression():
    ...
    tokens = tokenize("X()")
    ast, tokens = parse_simple_expression(tokens)
    assert ast == {"position": 0, "tag": "identifier", "value": "X"}
    pprint(ast)
...
def test_parse_while_statement():
    ...
    assert ast == {
        "condition": {"position": 6, "tag": "number", "value": 1},
        "do": {"position": 8, "tag": "number", "value": 2},
        "tag": "while",
    }
    # pprint(ast)
    #
    ...
    assert ast == {
        "condition": {
            "left": {"position": 6, "tag": "number", "value": 1},
            "right": {"position": 9, "tag": "number", "value": 1},
            "tag": "==",
        },
        "do": {
            "list": {
                ...
                "statement": {
                    "tag": "print",
                    "value": {"position": 27, "tag": "number", "value": 4},
                },
                "tag": "list",
            },
            "statement": {
                "tag": "print",
                "value": {"position": 18, "tag": "number", "value": 3},
            },
            "tag": "list",
        },
        "tag": "while",
